...
def infere(paths_list, output_dir, backend):
    already_processed_files = os.listdir(output_dir)

    # remove the prefix kc_ and not_kc_
    already_processed_files = [
        f.replace("results_openai", "") for f in already_processed_files
    ]
    already_processed_files = [
    ...
    for file_path in tqdm(left_files):
        try:
            ...
            # classify the case
            cot_facts_response = cot_facts(case_facts, backend=backend)
            cot_law_response = cot_law(case_law, backend=backend)
            cot_facts_law_response = cot_facts_law(
                case_facts, case_law, backend=backend
            )
