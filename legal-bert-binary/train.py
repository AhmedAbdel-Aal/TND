...
wandb.init(
    ...
    project="longformer-4096-facts-binary",
)
...
def compute_metrics(p):
    ...
    precision_macro = precision_score(labels, preds, average='binary')
    recall_macro = recall_score(labels, preds, average='binary')
    f1_macro = f1_score(labels, preds, average='binary')
    
    precision_micro = precision_score(labels, preds, average='binary')
    recall_micro = recall_score(labels, preds, average='binary')
    f1_micro = f1_score(labels, preds, average='binary')
...
model = LongformerForSequenceClassification.from_pretrained(model_name, num_labels=2, gradient_checkpointing=True)
 

from transformers import Trainer, TrainingArguments

training_args = TrainingArguments(
    output_dir="./results_longformer_binary",
    learning_rate=1e-5,
